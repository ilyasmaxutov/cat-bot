...
// Встроенный mapping команд → слова из таблицы
  ...
  try {
    ...
    if (key === '/reload') {
      if (!event.env || !event.env.SHEET_KV) {
        await sendTelegram(event.env, chatId, 'Binding "SHEET_KV" не найден.');
    ...
    if (answers && answers.length > 0) {
      ...
      await sendTelegram(event.env, chatId, 'Не знаю такого триггера 🙀 Напишите /reload, если вы только что добавили новый триггер.');
    }
  } catch (err) {
...
async function handleScheduled(event) {
  if (!event || !event.env || !event.env.SHEET_KV) {
